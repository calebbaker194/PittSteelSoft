-- Function: shipshipment(integer, timestamp with time zone)

-- DROP FUNCTION shipshipment(integer, timestamp with time zone);

CREATE OR REPLACE FUNCTION shipshipment(pshipheadid integer, ptimestamp timestamp with time zone)
  RETURNS integer AS
$BODY$
...
---End--------------------------------------------------------------------
	...
	IF ((_c.remain*100)/_c.original > 5) THEN
	  RETURN -99;
	END IF;
      END LOOP;
    END IF;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION shipshipment(integer, timestamp with time zone)
  OWNER TO admin;
